...
import net.gahvila.rtp.Distribution.DistributionShape;
...
public class WorldBorderPluginHook {
    ...
    public WorldBorderPluginHook(Server server) {
        ...
        if (hasHook()) {
            RTP.infoLog("It looks like your using the world border plugin '" + hook.name() + "'.");
            RTP.infoLog("Random teleport locations will always end up inside any world border.");
        }
    }
    ...
    private PluginSpecificHook findHook() {
        try {
            ...
            RTP.log(Level.WARNING, "You may want to manually set your RTP region for the time being.");
    ...
    class ChunkyBorderHook extends PluginSpecificHook {
        ...
        public Map<String, DistributionSettings> generateDistributions() {
            ...
            try {
                ...
                noLongerUsable = true;
                RTP.log(Level.WARNING, "Turns out there was an error with world border compatibility...");
                RTP.log(Level.WARNING, "No guarantees will be made about points being within the border.");
                e.printStackTrace();
            }
            return distributions;
        }
